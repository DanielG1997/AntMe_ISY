...
Application::Application() : window_(1200, 720) {}
    ...
    while (SDL_PollEvent(&event)) {
		...
		if (event.type == SDL_QUIT) {
			...
			switch (event.key.keysym.scancode) {
				case SDL_SCANCODE_W :
					cameratranslation = renderer.move(-0.05f, cameratranslation, camerarotation);
					break;
				...
				case SDL_SCANCODE_A:
					camerarotation = renderer.turn(1 * glm::pi<float>() / 180, camerarotation);
					break;
